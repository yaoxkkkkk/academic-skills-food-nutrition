...
- **Phase B · full text:** **obtain and read the full text** via the **`food-fetch`** skill (ladder in `references/full-text-access.md`) — **open-access items MUST be downloaded and read** (`scripts/fetch_oa.py`), then the user's reference-manager library / institutional session / provided PDFs. Read the semantically strong and borderline items; land ~15–30 (more for a systematic review). Reconciling metadata or reading snippets is **not** a substitute for reading an open-access paper. Where a source stays genuinely paywalled and unsupplied, screen it at **abstract-level and mark it** (not fully read); record the access route.
